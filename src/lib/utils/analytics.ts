...
export function buildKpis(
  ...
  return {
    revenue: {
      current: revenue,
      previous: pRevenue,
      delta: calcDelta(revenue, pRevenue),
      sparkline: spark,
    },
    transactions: {
      current: txns,
      previous: pTxns,
      delta: calcDelta(txns, pTxns),
      sparkline: pSpark,
    },
    ...
    margin:
      coverage > 0
        ? {
            current: margin,
            previous: pMargin,
            delta: calcDelta(margin, pMargin),
            coverage,
          }
        : null,
  };
...
function calcMargin(
  ...
  const coverage = items.length
    ? Math.round((withCost / items.length) * 100)
    : 0;
  const margin =
    revenue > 0 ? Math.round(((revenue - cogs) / revenue) * 1000) / 10 : 0;
...
export function buildPaymentMethods(sales: any[]): PaymentRow[] {
  ...
  for (const s of sales) {
    ...
    if (m === "credit") {
      // Split credit sales: paid portion → Cash, unpaid → Payment Due
      const paid = Number(s.credit_amount_paid ?? 0);
      const unpaid = Math.max(0, Number(s.total) - paid);
      if (!map.cash) map.cash = { revenue: 0, count: 0 };
      if (!map.credit) map.credit = { revenue: 0, count: 0 };
      map.cash.revenue += paid;
      map.credit.revenue += unpaid;
      map.cash.count += 1;
      map.credit.count += 1;
    } else {
...
export function buildWeekday(sales: any[], shopTz: string): TimeSlot[] {
  ...
  const slots: TimeSlot[] = DAYS.map((label) => ({
    label,
    revenue: 0,
    count: 0,
  }));
...
export function buildProducts(
  items: any[],
  productCostMap?: Map<string, number>,
): { byRevenue: ProductRow[]; byUnits: ProductRow[] } {
  ...
  for (const item of items) {
    ...
    if (cost > 0) {
      ...
      const itemMargin =
        itemRevenue > 0
          ? Math.round(((itemRevenue - itemCogs) / itemRevenue) * 100)
          : 0;
...
export function buildCustomerInsights(
  ...
  for (const c of customers) {
    const tier =
      c.visit_count >= 15 || c.total_spent >= 5000000
        ? "vip"
        : c.visit_count >= 8 || c.total_spent >= 1000000
          ? "regular"
          : "new";
    tiers[tier]++;
...
export function buildHeatmap(sales: any[], shopTz: string): number[][] {
  ...
  return accum.map((row) =>
    row.map((cell) =>
      cell.count > 0 ? Math.round(cell.total / cell.count) : 0,
    ),
  );
}
...
export function buildMonthCalendar(
  ...
  let total = 0,
    max = 0,
    bestDay: { date: string; value: number } | undefined;
  for (let i = 0; i < weeks * 7; i++) {
...
export function buildProfitCalendar(
  ...
  const buckets = new Map<
    string,
    { revenue: number; cogs: number; count: number; saleIds: Set<string> }
  >();
  const saleBuckets = new Map<
    string,
    Map<string, { revenue: number; cogs: number; createdAt: string }>
  >();
  ...
  for (const item of items ?? []) {
    ...
    b.revenue += lineTotal;
    b.cogs += itemCogs;
    if (!b.saleIds.has(item.sale_id)) {
      b.count += 1;
      b.saleIds.add(item.sale_id);
    }
    buckets.set(key, b);
  ...
  return {
    ...
    totalProfit,
    max: maxAbs,
    hasData: totalRev > 0 || totalCogs > 0,
    today: todayIso,
    bestDay,
    monthLabel: firstOf.format("MMMM YYYY"),
    daySales,
...
export function buildMargin(
  ...
  const daily = Object.entries(dayMap)
    ...
    .map(([date, d]) => {
      const _idx = dayjs(date).tz(shopTz).diff(fromD, "day");
  ...
  const marginPct =
    totalRev > 0 ? Math.round((grossProfit / totalRev) * 1000) / 10 : 0;
