...
export const GET = async ({
  ...
  const [
    ...
    supabase
      .from("sales")
      .select(
        "id, total, subtotal, tax_amount, payment_method, created_at, customer_id",
      )
      .eq("shop_id", shopId)
      .is("voided_at", null)
  ...
  const [{ data: allCurrentItems = [] }, { data: allCompareItems = [] }] =
    await Promise.all([
      supabase
        .from("sale_items")
        .select(itemSelect(""))
  ...
  const grossProfit = buildGrossProfit(
    saleItems,
    compareSaleItems,
    productCostMap,
  );
  ...
  const outstandingTotal = Number(
    (outstandingTotalRow as any)?.total_outstanding ?? 0,
  );
  const outstandingByStatusMap: Record<string, number> = {
    partial: 0,
    pending: 0,
  };
