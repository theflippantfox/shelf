...
export async function POST({
  ...
  const { data: sale, error: saleErr } = await supabase
    .from("sales")
    .select(
      "id, shop_id, total, payment_method, credit_status, credit_amount_paid, voided_at",
    )
    .eq("id", saleId)
    .single();
  ...
  const itemById = new Map(
    (saleItems ?? []).map((i: any) => [i.product_id, i]),
  );
  ...
  for (const r of (priorReturns ?? []) as any[]) {
    alreadyReturned[r.product_id] =
      (alreadyReturned[r.product_id] ?? 0) + r.qty;
  }
  ...
  for (const it of items) {
    ...
    if (!orig)
      throw error(400, `Product ${it.product_id} was not on this sale`);
    ...
    if (it.qty > remain) {
      throw error(
        400,
        `Can only return ${remain} of ${(orig as any).product_name} (already returned ${prior})`,
      );
    }
  ...
  const { data: ret, error: retErr } = await supabase
    ...
    .single();
  if (retErr || !ret)
    throw error(500, retErr?.message ?? "Could not create return");
  ...
  for (const it of items) {
    const isRestock =
      it.condition === "resellable" || it.condition === "expired";
    if (isRestock) {
