...
export async function GET({
  ...
  const { data, error } = await supabase
    .from("shop_members")
    .select(
      "id, role, status, permissions, created_at, invited_at, invited_by, user:profiles!shop_members_user_id_fkey(id, first_name, last_name, avatar_url)",
    )
  ...
  if (userIds.length) {
    const { data: rows } = await admin.rpc(
  ...
  const inviterIds = [
    ...new Set(members.map((m) => m.invited_by).filter(Boolean)),
  ];
