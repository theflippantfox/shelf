...
export async function POST({
  ...
  if (request.body) {
    try {
      ...
      // Empty body is fully optional for this endpoint.
    }
  }
  ...
  const update: Record<string, unknown> = { sharing_enabled: enable };
  if (enable) {
...
export async function GET({
  ...
  const { data, error: rErr } = await (supabase as any)
    ...
    .single();
  if (rErr || !data)
    throw error(500, rErr?.message ?? "Could not read share settings");
