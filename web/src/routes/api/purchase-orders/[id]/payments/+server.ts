...
export async function POST({
  ...
  if (!PO_PAYMENT_METHODS.includes(method as any)) {
  ...
  if (method === PAYMENT_METHOD.CASH || method === PAYMENT_METHOD.BANK) {
    try {
      ...
      if (regErr) {
        console.warn(
          "[po-payments] cash_register insert failed:",
          regErr.message,
        );
      } else if (reg) {
  ...
  if (payErr) {
    ...
    if (registerEntryId) {
      try {
        ...
        if (compErr)
          console.warn(
            "[po-payments] compensation insert failed:",
            compErr.message,
          );
        else
          console.warn(
            "[po-payments] wrote compensating register row",
            (comp as any)?.id,
          );
      } catch {
        /* swallow */
  ...
  if (
    Math.abs(newPaidTotal - total) < 0.005 &&
    (po as any).status !== PO_STATUS.RECEIVED
  ) {
